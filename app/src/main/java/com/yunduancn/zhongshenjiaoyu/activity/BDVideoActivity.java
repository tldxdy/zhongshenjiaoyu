...
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.BDWidget.BDCloudVideoView;
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.bar.AdvancedMediaController;
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.info.DownloadObserverManager;
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.info.SampleObserver;
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.info.SharedPrefsStore;
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.info.VideoInfo;
import com.yunduancn.zhongshenjiaoyu.BaiduVideo.popview.FullScreenUtils;
...
public class BDVideoActivity extends Activity implements IMediaPlayer.OnPreparedListener,
     ...
     * 记录播放位置
     */
    private int mLastPos = 6*60*1000;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bdvideo);
...
//        mVV.setMaxProbeTime(1 * 1000); // 设置首次缓冲的最大时长
        ...
        mVV.setVideoPath("http://edu.yunduancn.cn/mapi_v1/courses/56/lessons/533/media?token=");

        mVV.setInitPlayPosition(mLastPos);
    ...
    private void initOtherUI() {
        ...
        ibBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(isFullScreen){
                    setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                    FullScreenUtils.toggleHideyBar(BDVideoActivity.this);
                    ...
                    isFullScreen = false;
                    BDVideoActivity.this.findViewById(R.id.ibtn_screen_control).setBackgroundResource(R.drawable.btn_to_fullscreen);
                }else{
                    finish();
                }
            }
        ...
        mediaController.setDownloadListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (info.getUrl().startsWith("file://")) {
                    Toast.makeText(BDVideoActivity.this, "该资源已经是本地文件", Toast.LENGTH_SHORT).show();
                    return;
                } else if (!info.getUrl().endsWith(".m3u8")) {
                    Toast.makeText(BDVideoActivity.this, "抱歉，当前仅支持m3u8资源的下载", Toast.LENGTH_SHORT).show();
                    return;
                }
                VideoDownloadManager downloadManagerInstance = VideoDownloadManager
                        .getInstance(BDVideoActivity.this, BDVideoActivity.SAMPLE_USER_NAME);
                DownloadableVideoItem item = downloadManagerInstance
                        .getDownloadableVideoItemByUrl(info.getUrl());
                if (item != null) {
                    // already
                    Toast.makeText(BDVideoActivity.this, "该资源已经在缓存列表，请到「本地缓存」查看", Toast.LENGTH_SHORT).show();
                } else {
                    SharedPrefsStore.addToCacheVideo(BDVideoActivity.this, info);
                    ...
                    Toast.makeText(BDVideoActivity.this, "开始缓存，可到「本地缓存」查看进度", Toast.LENGTH_SHORT).show();
                }
            }
